...
def calculate_metric(args, _real_data, _synthetic, sensitive_attributes=None):
    ...
    if cont_attributes:
        real_data_no_sens = pd.concat([real_data_no_cont, real_data[cont_attributes]], axis=1)
        syn_data_no_sens = pd.concat([syn_data_no_cont, syn_data[cont_attributes]], axis=1)
    else:
